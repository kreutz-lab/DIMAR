...
if (method=='midastouch' || method=='rf' || method=='mean' || method=='norm' || method=='ri' || method=='pmm' || method=='sample' || method=='cart')
  ...
  { lib <-'Hmisc'}
else if (method=='ppca' || method=='bpca' || method=='nipals' || method=='nlpca' || method=='svd' || method=='svdImpute')
  { lib <-'pcaMethods'}
else if (method=='MinDet' || method=='KNN' || method=='MinProb' || method=='QRILC')
  { lib <-'imputeLCMD'}
else if (method=='SVTApproxImpute' || method=='SVTImpute' || method=='SVDImpute' || method=='kNNImpute' || method=='lmImpute')
  { lib <-'imputation'}
else if (method=='missForest')
  { lib <-'missForest'}
else if (method=='softImpute') { lib <-'softImpute'}
else if (method=='irmi') { lib <-'VIM'}
else if (method=='Seq' || method=='SeqRob' || method=='impSeq' || method=='impSeqRob') 
  { lib <-'rrcovNA'}
else if (method=='MIPCA' || method=='imputePCA')
  { lib <-'missMDA'}
else if (method=='mi') { lib <-'mi'}
else if (method=='knnImputation'){ lib <-'DMwR'}
